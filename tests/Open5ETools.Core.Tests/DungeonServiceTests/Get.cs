...
public class Get(TestFixture fixture) : IClassFixture<TestFixture>
{
    ...
    {
        const int userId = 3;
        var result = await _dungeonService.GetAllDungeonOptionsForUserAsync(userId, TestContext.Current.CancellationToken);
    ...
    public async Task GetDungeonOptionByNameAsync_WithValidName_ReturnsDungeonOptionModel()
    {
        ...
        var result =
            await _dungeonService.GetDungeonOptionByNameAsync(option.DungeonName, option.UserId,
