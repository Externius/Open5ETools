...
public class DungeonNoCorridor(IDungeonHelper dungeonHelper) : Dungeon(dungeonHelper), IDungeonNoCorridor
{
    ...
    private void FillDoor(int down, int right)
    {
        ...
        do
        {
            var random = DungeonHelper.GetRandomInt(0, _edgeTileList.Count);
    ...
    {
        var down = DungeonHelper.GetRandomInt(2, Math.Abs(vertical) > RoomSize ? RoomSize : Math.Abs(vertical));
        var right = DungeonHelper.GetRandomInt(2, Math.Abs(horizontal) > RoomSize ? RoomSize : Math.Abs(horizontal));
        if (vertical < 0)
            down = -down;
        if (horizontal < 0)
            right = -right;
    ...
    public void AddFirstRoom()
    {
        ...
        var down = DungeonHelper.GetRandomInt(2, RoomSize + 1);
    ...
    protected override int GetDoorCount(int down, int right)
    {
        if (Math.Abs(down) < 4 || Math.Abs(right) < 4)
            return 2;
        return DungeonHelper.GetRandomInt(3, 6);
    }
    ...
    {
        if (DungeonHelper.GetRandomInt(0, 101) < 40)
            DungeonTiles[x][y].Texture = Texture.NoCorridorDoorTrapped;
        else if (DungeonHelper.GetRandomInt(0, 101) < 50)
