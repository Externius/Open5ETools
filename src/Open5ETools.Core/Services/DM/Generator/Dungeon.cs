...
public class Dungeon(IDungeonHelper dungeonHelper) : IDungeon
{
    ...
    public void AddCorridorItem(int inCount, Item item)
    {
        ...
        while (inCount > count)
        {
            var x = DungeonHelper.GetRandomInt(0, _corridors.Count);
    ...
    private List<DungeonTile> GenerateDeadEnds()
    {
        ...
        do
        {
            var tile = dungeonList[DungeonHelper.GetRandomInt(0, dungeonList.Count)];
    ...
    {
        if (down < 4 || right < 4)
            return DungeonHelper.GetRandomInt(1, 3);
        return DungeonHelper.GetRandomInt(2, 5);
    }
    ...
    private int[] SetTilesForRoom()
    {
        ...
        do
        {
            x = DungeonHelper.GetRandomInt(3, max); // 3 because of edge + room_edge
